Here's the actual manual process - what you're describing (shortcut + drop the app on it) is really just the *quick test launch* trick, not real bundling. Real bundling produces one self-contained package you can hand to someone else. Here's how:

**Step 1 - Get the right build**
Download the **Normal** (non-SDK) NW.js build for whatever platform you're targeting from nwjs.io/downloads. Unzip it - you'll see `nw.exe` plus a handful of companion files (`.dll` files, `nw.pak`, `icudtl.dat`, a `locales` folder, etc.). Keep all of those together; you'll need the whole folder, not just the exe.
